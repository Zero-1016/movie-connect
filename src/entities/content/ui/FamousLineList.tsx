...
import { getMovieFamousLineList } from '@/entities/local_movie/api'
...
export function FamousLineList({ movieId }: Props) {
  const { curPage, setCurPage } = usePagination(1)
  const { data, refetch } = useSuspenseQuery<ContentResponse>({
    queryKey: LOCAL_QUERY_KEY.movieFamousLine(movieId),
    queryFn: () => getMovieFamousLineList(movieId, curPage),
  })
