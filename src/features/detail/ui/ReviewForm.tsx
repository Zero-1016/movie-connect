...
import { postReview } from '@/features/detail/lib'
...
export function ReviewForm({ movieId }: Props) {
  const onSubmit: SubmitHandler<z.infer<typeof contentFormSchema>> = async ({ content }) => {
    try {
      await postReview({ movieId, content })
      reset()
    } catch (err) {
  ...
  return (
    <form className={classNames(styles.formContainer, notoSans.className)} onSubmit={handleSubmit(onSubmit)}>
      <label className={styles.label}>
        <TextareaAutosize className={styles.textArea} placeholder="리뷰 작성하기" {...register('content')} />
      </label>
